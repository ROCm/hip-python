...
cdef class HipModuleLaunchKernel_extra(rocm.bindings.util.types.Pointer):
    ...
    * `object` that is accepted as input by `~.Pointer.__init__`:
    ...
    def __cinit__(self):
        ...
        self._config[0] = <void*>rocm.bindings.cyhip.HIP_LAUNCH_PARAM_BUFFER_POINTER
        self._config[2] = <void*>rocm.bindings.cyhip.HIP_LAUNCH_PARAM_BUFFER_SIZE
        self._config[3] = <void*>&self._args_buffer_size
        self._config[4] = <void*>rocm.bindings.cyhip.HIP_LAUNCH_PARAM_END

    @staticmethod
    cdef HipModuleLaunchKernel_extra fromPtr(void* ptr):
        cdef HipModuleLaunchKernel_extra wrapper =\
            HipModuleLaunchKernel_extra.__new__(HipModuleLaunchKernel_extra)
        wrapper._ptr = ptr
        return wrapper

    cdef size_t _aligned_size(self, size_t size, size_t factor):
        return factor * ((size) // (factor) + ((size) % (factor) != 0))  # //: floor division  # no-cython-lint
        ...
        if isinstance(pyobj, HipModuleLaunchKernel_extra):
            ...
            for entry in pyobj:
                if isinstance(entry, ctypes_types):
                    buffer_size += self._aligned_size(ctypes.sizeof(entry),
                                                      MAX_ALIGN)  # overestimate
                else:
                    ptr = rocm.bindings.util.types.Pointer(entry)._ptr  # fails if cannot be converted to pointer # no-cython-lint
            ...
            for entry in pyobj:
                ...
                self._args_buffer_size = self._aligned_size(self._args_buffer_size,
                                                            min(num_bytes, MAX_ALIGN))
                libc.string.memcpy(<void*>&args[self._args_buffer_size], ptr, num_bytes)
                self._args_buffer_size += num_bytes  # TODO: self._aligned_size(num_bytes)  # must be 8-byte aligned  # no-cython-lint
        else:
            rocm.bindings.util.types.Pointer.init_from_pyobj(self, pyobj)
    ...
    cdef HipModuleLaunchKernel_extra fromPyobj(object pyobj):
        ...
        In case ``pyobj`` is itself a ``HipModuleLaunchKernel_extra`` instance, this
        method returns it directly. No new ``HipModuleLaunchKernel_extra`` is created.

        Args:
            pyobj (`object`):
                Must be either ``None``, a simple, contiguous buffer according to the
                buffer protocol, or of type ``_util.types.Pointer``,
                ``HipModuleLaunchKernel_extra``, ``int``, or ``ctypes.c_void_p``.

                Furthermore, ``pyobj`` can be a list or tuple in the following shape,
                where each entry can be either be:

                * (1) pointer-like, i.e., can be translated to ``_util.types.Pointer``,
        ...
        """
        cdef HipModuleLaunchKernel_extra wrapper = HipModuleLaunchKernel_extra.__new__(
            HipModuleLaunchKernel_extra)
