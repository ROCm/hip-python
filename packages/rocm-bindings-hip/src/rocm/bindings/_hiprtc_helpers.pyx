...
cimport rocm.bindings.cyhiprtc
cimport libc.stdlib
cimport libc.string
...
cdef class HiprtcLinkCreate_option_ptr(rocm.bindings.util.types.Pointer):
    ...
    * `object` that is accepted as input by `~.Pointer.__init__`:
    ...
    cdef HiprtcLinkCreate_option_ptr fromPtr(void* ptr):
        cdef HiprtcLinkCreate_option_ptr wrapper = HiprtcLinkCreate_option_ptr.__new__(
            HiprtcLinkCreate_option_ptr)
        wrapper._ptr = ptr
        return wrapper
    ...
    cdef void init_from_pyobj(self, object pyobj):
        ...
        cdef size_t entry_size = sizeof(unsigned int)  # see: hiprtc.hiprtcJIT_option.ctypes_type() #  no-cython-lint
        ...
        if isinstance(pyobj, HiprtcLinkCreate_option_ptr):
            ...
            element_ptr = <unsigned int*>self._ptr  # see: hiprtc.hiprtcJIT_option.ctypes_type() #  no-cython-lint

            # 3. Copy the data into the struct and calculate the offset
            for i in range(0, num_entries):
                entry = pyobj[i]
                if not isinstance(entry, hiprtc.hiprtcJIT_option):
                    raise ValueError(
                        "list/tuple entries must be of type"
                        + "'hiprtc.hiprtcJIT_option'"
                    )
                element_ptr[i] = <unsigned int>cpython.long.PyLong_AsUnsignedLong(
                    entry.value)
        else:
    ...
    cdef HiprtcLinkCreate_option_ptr fromPyobj(object pyobj):
        ...
        In case ``pyobj`` is itself a ``HiprtcLinkCreate_option_ptr`` instance, this
        method returns it directly. No new ``HiprtcLinkCreate_option_ptr`` is created.

        Args:
            pyobj (`object`):
                Must be either ``None``, a simple, contiguous buffer according to the
                buffer protocol, or of type ``_util.types.Pointer``,
                ``HiprtcLinkCreate_option_ptr``, ``int``, or ``ctypes.c_void_p``

                Furthermore, ``pyobj`` can be a list or tuple in the following shape,
                where each entry must be of type `~.hiprtcJIT_option`.
