...
class HSAKernel(HSAKernelBase):
    ...
    def _parse_kernel_resource(self):
        ...
        #: Following `https://llvm.org/docs/AMDGPUUsage.html#code-object-metadata`
        #: We replaced `wavefront_sgpr_count` by `sgpr_count`
        #: We replaced `workitem_vgpr_count` by `vgpr_count`
        m = re.search(r"\bsgpr_count\s*:\s*(\d+)", self.assembly)
        self._wavefront_sgpr_count = int(m.group(1))
        m = re.search(r"\bvgpr_count\s*:\s*(\d+)", self.assembly)
