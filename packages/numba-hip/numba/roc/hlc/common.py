...
def adapt_llvm_datalayout(llvmir: str):
    """
    Adapt the LLVM IR to match the syntax required by HLC.
    """
    
    #   return add_metadata_type(llvmir)
    return llvmir.replace(config.DATALAYOUT[64],config.AMDGCN_BC_DATALAYOUT[64])
...
class AMDGCNModule(object):
    ...
    # Obtained via: `hipcc -### HelloWord.hip`
    # FIXME move out, make configurable
    _BITCODES = [
      "hip.bc",
      "ocml.bc",
      "ockl.bc",
      ...
      "oclc_isa_version_{arch}.bc",
      "oclc_abi_version_{abi_ver}.bc",
    ]

    def iter_bitcodes(self,arch,abi_ver="500"):
        """Yields the bitcodes.
        
        Note:
            The `gfx` part of the arch identifiers `gfx[0-9]+` must be stripped away.
        """
        for bc in self._BITCODES:
            yield bc.format(arch=arch[3:].lower(),abi_ver=abi_ver)
