...
class _Kernel(serialize.ReduceMixin):
    """
    HIP Kernel specialized for a given set of argument types. When called, this
    ...
    def __init__(
        ...
        # HIP target, _Kernel instances are stored instead, so we provide this
        # attribute here to avoid duplicating nopython_signatures() in the HIP
        ...
        lib, kernel = tgt_ctx.prepare_hip_kernel(
            cres.library,
            cres.fndesc,
            debug,
            lineinfo,
...
class HIPCacheImpl(CacheImpl):
    ...
    def check_cachable(self, cres):
        # HIP Kernels are always cachable - the reasons for an entity not to
        # be cachable are:
        #
        # - The presence of lifted loops, or
        # - The presence of dynamic globals.
        #
        # neither of which apply to HIP kernels.
        return True


class HIPCache(Cache):
    """
    Implements a cache that saves and loads HIP kernels and compile results.
    """

    _impl_class = HIPCacheImpl


class HIPDispatcher(Dispatcher, serialize.ReduceMixin):
    """
    HIP Dispatcher object. When configured and called, the dispatcher will
    specialize itself for the given arguments (if no suitable specialized
    version already exists) & compute capability, and launch on the device
    associated with the current context.

    Dispatcher objects are not to be constructed by the user, but instead are
    created using the :func:`numba.hip.jit` decorator.
    """

    # Whether to fold named arguments and default values. Default values are
    # presently unsupported on HIP, so we can leave this as False in all
    # cases.
    _fold_args = False

    targetdescr = hip_target

    def __init__(self, py_func, targetoptions, pipeline_class=HIPCompiler):
        super().__init__(
            py_func, targetoptions=targetoptions, pipeline_class=pipeline_class
        )

        # The following properties are for specialization of HIPDispatchers. A
        # specialized HIPDispatcher is one that is compiled for exactly one
    ...
    def _numba_type_(self):
        return hip_types.HIPDispatcher(self)

    def enable_caching(self):
        self._cache = HIPCache(self.py_func)
    ...
    def forall(self, ntasks, tpb=0, stream=0, sharedmem=0):
        ...
        - the kernel maps the Global Thread ID ``hip.grid(1)`` to tasks on a
          1-1 basis.
    ...
    def extensions(self):
        ...
        - `stream` the HIP stream used for the current call to the kernel
